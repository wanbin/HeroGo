...
- (id)initWithAppKey:(NSString *)appKey appSecret:(NSString *)appSecret text:(NSString *)text image:(UIImage *)image 
{
    if (self = [super initWithFrame:CGRectMake(0, 0, 320, 480)])
    {
		...
		baseScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, 320, 480)];
		baseScrollView.contentSize = CGSizeMake(320, 481);
		...
		[baseScrollView addSubview:panelView];
...
- (void)dealloc
{
    ...
    [baseScrollView release],baseScrollView = nil;
...
- (void)onSendButtonTouched:(id)sender
{
	NSString * temStr = [self getSendMessage];
	...
	else if ([self textLength:temStr]>140) {
		UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"新浪微博", nil)
                                                             message:NSLocalizedString(@"好友貌似有点多呀，减点吧", nil)
                                                            delegate:nil
                                                   cancelButtonTitle:NSLocalizedString(@"确定", nil) otherButtonTitles:nil];
		[alertView show];
		[alertView release];
    ...
    [engine sendWeiBoWithText:temStr image:contentImage];
}
